...
export default async (app: Application) => {
    app.get('/get-address/:addressId', async (request: Request, response: Response, next: NextFunction) => {
        try {
            const addressId = request.params.addressId as string;

            RepositoryContext.GetInstance().addressRepository.getAddress(addressId)
                .then((result) => {
